...
格式参考：`.self-workflow/templates/analysis-template.md`
...
格式参考：`.self-workflow/templates/adr-template.md`
...
审查报告格式参考：`.self-workflow/templates/review-report-template.md`
...
格式参考：`.self-workflow/templates/summary-template.md`
---
...
创建模板参考：`.self-workflow/templates/workflow-metadata-template.yaml`

#### 执行流程

1. **启动**：复制 `workflow-metadata-template.yaml` 到 `artifacts/<workflow-id>/workflow.yaml`，填入具体值
...
- **错误详细日志**：写入 `errors/<workflow-id>/01-analysis-errors.md`（按阶段）或 `errors/<workflow-id>/02-design-errors.md` 等
- **错误索引**：同步更新 `errors/<workflow-id>/errors.yaml`，格式参考 `.self-workflow/templates/workflow-metadata-template.yaml` 中的 error 定义
