...
- [ ] 不纳入范围已明确标注
- [ ] **决策捕捉**：检查 adrs/ 目录——若本阶段产出触及 decision-record.md 的触发场景，adrs/ 下必须有对应 ADR 文件（非空，含来源引用+决策理由）；若未触及，需显式标注判断依据
...
2. **产物自查（MUST）**：
   ...
   - [ ] 模板一致性：内容结构与 `analysis-template.md` 一致
...
4. 约束条件是否遗漏了已知的项目规范？
5. **Gate 纪律检查**：主 Agent 是否在本 Gate 入口执行了前置检查（量化输出+产物自查+自检清单）？如未执行，标注为 behavior: warning
...
**MUST NOT** 在 review-agent 返回结果前推进至下一阶段。后台审查≠跳过 Gate——必须等待审查结果返回，根据结果决定通过/修复。
...
- [ ] 数据模型已设计
- [ ] 备选方案至少 2 个
- [ ] **决策捕捉**：已检查 adrs/ 目录——若触及触发场景则必有文件，否则显式标注判断依据
...
2. **产物自查（MUST）**：
   逐项对照产物模板/规范检查阶段产物：
   - [ ] 产物完整性：所有 required-fields 均已填写
   - [ ] 模板一致性：内容结构与 `design-template.md` 一致
   - [ ] 跨阶段一致性：未推翻 Phase 1 已确认的约束条件和不纳入范围（如有推翻，ADR 中需有显式反转说明）
...
方向审查不通过 → 返回方案设计阶段修正，不进入后续步骤。**MUST NOT** 在方向审查未通过时进入 Phase 3。
...
task(subagent_type="review-agent", prompt="执行对抗性设计评审(Grill风格)。对设计文档 <artifacts/02-design.md> 的每个架构决策：分析备选方案是否充分评估、检查选择的理由是否成立、挑战隐含假设、确认trade-off被明确记录。同时检查推理链一致性：交叉对照 Phase 1 的约束条件和不纳入范围（<artifacts/01-analysis.md>），如设计方案推翻了前阶段结论，确认 ADR 中有显式的反转说明。**Gate 纪律检查**：主 Agent 是否执行了前置检查、方向审查（步骤 0）？是否在方向审查未通过时仍继续？如违反，标注为 behavior: failed。输出审查报告(YAML格式)，每个决策 passed/warning/failed，含 behavior 维度评估。")
...
**MUST NOT** 在方向审查或对抗性审查未通过时进入 Phase 3。
...
2. **产物自查（MUST）**：
   逐项对照产物模板/规范检查阶段产物：
   - [ ] 产物完整性：所有 required-fields 均已填写
   - [ ] 模板一致性：内容结构与 `implementation-template.md` 一致
   - [ ] 跨阶段一致性：实现内容与设计文档的接口定义、数据模型逐项对照无偏差
...
程序化验证不通过时 **MUST NOT** 跳过修复直接提交对抗性审查。
...
6. **Gate 纪律检查**：主 Agent 是否在编码前经用户确认了修改方案？是否执行了前置检查？程序化验证跳过时是否声明了理由？如违反任一，标注为 behavior: warning/failed
...
2. **产物自查（MUST）**：
   逐项对照产物模板/规范检查阶段产物：
   - [ ] 产物完整性：所有 required-fields 均已填写
   - [ ] 模板一致性：内容结构与 `verification-template.md` 一致
   - [ ] 跨阶段一致性：验证结果与 Phase 1 的验收标准逐条对照无遗漏
...
5. **Gate 纪律检查**：主 Agent 是否执行了前置检查？验证是否仅做文件存在性检查而无实质验证？程序化验证跳过时是否声明了理由？如违反，标注为 behavior: warning
...
**MUST NOT** 将仅做文件存在性检查的结果标记为"验证通过"（必须包含实质验证步骤）。
