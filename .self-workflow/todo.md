...
6. 门控机制优化——文档变更和新增也需要进行审查（来源：新增 #3）

7. 关键决策自动记录→通用 spec——当前决策捕捉不理想，要做成可复用的通用 spec（来源：新增 #4）

8. 经验沉淀重要程度评估——不仅判断"能不能复用"，还要评估"对项目推进的帮助性"（来源：新增 #7）

9. docs/ 文档 tag 质量治理——统一中英文、同义词→同一 tag、消除大小写不一致（来源：V1.7 实施 + 新增 #9）

10. 文档交叉引用机制——frontmatter 中加 `related:` 字段，Agent 按需追踪关联文档（来源：V1.7 经验沉淀 + 新增 #10）

11. 渐进式披露触发条件细化——READMe.md 每个分类增加"触发场景"描述（来源：V1.7 注入机制讨论 + 新增 #11）
...
5. todo 整理排期 command——Agent 可自主整理 todo、排期，减少 Human 手动维护（来源：新增 #1）

6. task 模板从 feat command 移动到 templates——可由用户进行定制（来源：新增 #2）

7. /feat 增强——从 todo 领取任务结束后自动更新 todo 状态（来源：新增 #8）

8. packages/installer/.opencode .self-workflow templates 三层目录关系梳理优化（来源：新增 #5）

9. 各阶段验证能力增强——尤其对抗性审查，不要默认正确，用质疑的眼光审视结果（来源：新增 #6）
...
- 经验如何复利——刷新/去重/过时标记/晋升（来源：todo #7 完整版 + 新增 #12 freshness） <放到V1.10 Skill优化>
...
- tag 权重/层级体系——同 tag 多文档时帮助 Agent 择优匹配（来源：新增 #13） <放到V2.0 经验体系>
