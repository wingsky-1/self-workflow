...
## V1.16：经验检测（P2）🟢 — ✅ 已完成 (feat-经验检测-沉淀质量-20260607)
...
## V1.17：经验沉淀质量（P2）🟢 — ✅ 已完成 (feat-经验检测-沉淀质量-20260607)
...
> P1: 修复 Phase 4→5 文档更新步骤缺失；P2: 减少上下文污染。
...
> 新 command + session 辨识度改善。

1. 增加周报/日报 command——方便回顾和汇报进展
   → 来源：新增 #4

2. /feat 开始时 session 重命名为任务名称——提高 session 辨识度
   → 来源：新增 #5
...
1. 结合 docs\迭代报告 优化框架自带的 spec command agent skill hook 等描述

2. [流程] Phase Gate 纪律强化——Gate 未通过绝对不进入下一阶段，后台审查≠跳过 Gate
   → 来源：V1.16+V1.17 会话评审 #1

3. [流程] 产物自查——提交 Gate 审查前，逐项对照设计文档/模板/规范检查一致性（降低返工率）
   → 来源：V1.16+V1.17 会话评审 #6（一次正确率 33%）

4. [流程] ADR 先于产物——先创建 ADR 文件(按模板)，产物文档仅引用摘要不内联
   → 来源：V1.16+V1.17 会话评审 #4（重复犯"内联ADR"错误）

5. [规范] doc-audience 术语检查——新建 spec/docs 时检查术语是否为领域已有概念（非自创词）
   → 来源：V1.16+V1.17 会话评审 #9（"策展"→"治理"）

6. [工具] todowrite 即时启动——Phase 1 入口即创建条目，不延迟到用户提醒后
   → 来源：V1.16+V1.17 会话评审 #3

7. [流程] Gate 提交前自检清单——Agent 提交 Gate 审查前 MUST 检查：task.yaml structure 已同步、ADR frontmatter/引用完整、产物 frontmatter 合规
   → 来源：V1.15 会话评审（36%一次正确率）+ V1.16+V1.17 会话评审 #6（33%一次正确率），共同模式：文档合规类缺陷占比最高

8. [工具] 内置工具优先——Agent 执行操作前应检查项目是否有内置工具（如 sw_task_* 系列），避免手工重复实现
