...
> 减少不必要的打断，让 Agent 更自主。内置工具化让确定性操作由代码完成。
...
5. todo 系统注入机制——确定 todo 介绍注入到哪些场景（主Agent / feat command / 子Agent）
   → 来源：新增 #5

6. 歧义澄清 spec——Agent 遇到歧义输入时必须质疑询问，不自作主张 → 沉淀为 default/ spec
   → 来源：新增 #8

7. /feat 无人值守模式——Agent 自主完成 feat，用户事后评审
   → 来源：新增 #10

8. 内置 tool 化——确定性操作由内置 tool 实现（获取未完成 task 列表、创建 task 目录等），减少 token 浪费
   → 来源：新增 #7
