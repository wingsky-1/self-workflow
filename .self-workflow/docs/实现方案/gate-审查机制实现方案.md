...
Gate 入口：量化 scope+risk+user-signal → weight (skip/light/full)
    │
    ▼
前置检查（所有 weight 均执行，不可跳过）
    ├── 量化输出（即使 weight=skip 也不可省略）
    ├── 产物自查（对照模板/规范检查一致性）
    ├── 自检清单（task.yaml 同步 + ADR 完整 + frontmatter 合规）
    └── 程序化验证声明（执行或显式跳过附理由）
    │
    ├── weight=skip ──→ 跳过后续审查步骤，直接通过
    │
    ...
    └── weight=full ──→ 完整审查：
         ├── 步骤 0：方向审查（仅 Gate 2）
         ├── 步骤 1：程序化验证
         ├── 步骤 2：对抗性审查（review-agent，含 Gate 纪律检查）
         └── 步骤 3：人工确认（Gate 1/2）
              │
              ▼
...
| Gate | 前置检查 | 方向审查 | 程序化验证 | 对抗性审查（含纪律检查） | 人工确认 |
|------|---------|---------|-----------|----------------------|---------|
| Gate 1（分析） | ✅ 4 项 | — | 跳过 | ✅ 需求完整性 + Gate 纪律 | ✅ 需要 |
| Gate 2（设计） | ✅ 4 项 | ✅ 架构一致性 | 跳过 | ✅ Grill 风格 + behavior + Gate 纪律 | ⚠️ 可选 |
| Gate 3（实现） | ✅ 4 项 | — | ✅ lint/typecheck/test | ✅ 设计一致性 + Gate 纪律 | ❌ |
| Gate 4（验证） | ✅ 4 项 | — | ✅ 完整测试套件 | ✅ 验证真实性 + Gate 纪律 | ❌ |
...
| 2026-06-07 | feat-Gate流程纪律强化-20260607 | V1.21：新增前置检查段（量化输出 + 产物自查 + 自检清单 + 程序化验证声明），各 Gate 增加 MUST NOT 阻断式措辞，review-agent 提示词增加 Gate 纪律检查项 |
