...
```yaml
---
name: exp-curation              # MUST 与目录名一致
description: "经验策展 skill——审查经验文档质量、检测重复、评估生命周期。Triggers: 'exp-curation', '经验策展', '审查经验', 'exp-review'"
---
```

**目录结构**：
```
.opencode/skills/exp-curation/
└── SKILL.md              ← skill 指令文件

加载方式：
skill(name="exp-curation") → Agent 加载 SKILL.md 内容到上下文
```
...
type: spec
level: default
tags: [exp-curation, quality, lifecycle, dedup, review]
version: 1.0.0
summary: "经验文档质量分级、生命周期管理、去重审查、策展流程——定义 Agent 如何审查和维护经验库"
---
```
...
## 核心原则
经验资产需要持续策展才能复利。策展 = 质量审查 + 去重 + 生命周期管理。

## MUST

### 审查触发
- Phase 5 总结沉淀时，载入 exp-curation skill 前必须加载本 spec 作为判断依据
- 用户说"审查经验"或类似指令时，Agent 应自动加载本 spec + exp-curation skill
...
## 关联
- `exp-curation` skill：策展执行指令
- `feat-workflow.md` Phase 5：策展触发点
...
**插入位置**：
- **执行内容列表**（编写总结/经验草稿/...）：新增"经验策展"执行动作——在"经验草稿"之前
- **完成检查清单**（task级经验/doc级经验/...）：新增"经验策展已完成"检查项——在"doc 级经验"之前

```markdown
# 执行内容新增：
- [ ] **经验策展**：加载 `exp-curation` skill（`skill(name="exp-curation")`），
  对 `.self-workflow/docs/` 执行审查：检查现有经验的质量、检测与即将写入的新经验是否有重复、
  识别可晋升或应标记过时的文档。审查结果写入 `05-summary.md` 的经验策展节。

# 完成检查清单新增：
- [ ] **经验策展已完成**：exp-curation skill 已执行，审查结果已写入 summary
```
...
[".opencode/skills/exp-curation/SKILL.md", "skills/exp-curation/SKILL.md"]
```
...
```
packages/installer/index.js MANIFEST 新增：
[".self-workflow/specs/default/exp-curation.md", "specs/default/exp-curation.md"]
```
...
spec `exp-curation.md` 定义 5 级生命周期：`draft → verified → outdated → refreshed → archived`。
...
| 文件 | 位置（模板源） | 部署位置 |
|------|-------------|---------|
| SKILL.md | `packages/installer/templates/skills/exp-curation/SKILL.md` | `.opencode/skills/exp-curation/SKILL.md` |
| exp-curation.md | `packages/installer/templates/specs/default/exp-curation.md` | `.self-workflow/specs/default/exp-curation.md` |
...
| 步骤 | 内容 | 依赖 |
|------|------|------|
| 1 | 创建 `specs/default/exp-curation.md`（模板源） | — |
| 2 | 创建 `skills/exp-curation/SKILL.md`（模板源） | 需了解 OpenCode skill 格式（已完成） |
...
| 6 | 验证：skill 可被 `skill(name="exp-curation")` 加载 | 步骤 5 |
