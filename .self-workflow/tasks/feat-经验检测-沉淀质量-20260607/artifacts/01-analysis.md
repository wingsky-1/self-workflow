...
| # | 功能点 | 说明 | 来源 |
|---|--------|------|------|
| F1 | 经验去重检测 | 自动扫描 `.self-workflow/docs/`，**两级检测**：①元数据初筛（标题编辑距离≤3、tag重叠率≥80%、category相同）→ ②内容深比较（对初筛候选做文本相似度比对，如 SimHash/MinHash），报告疑似重复 | todo #7（原 V1.13-1） |
...
- [ ] A1.1：存在两级去重检测机制：
  - **第一级（元数据初筛）**：标题编辑距离 ≤ 3、tag 重叠率 ≥ 80%、或 category 相同+标题高度相似
  - **第二级（内容深比较）**：对通过初筛的候选对做文本相似度比对（SimHash/MinHash），内容相似度 ≥ 阈值（可调，默认 0.7）的标记为"疑似重复"
  - **Given** `.self-workflow/docs/` 下有文档
  - **When** 执行去重检测
  - **Then** 输出通过两级检测的疑似重复文档对，含元数据相似度 + 内容相似度指标（阈值可在设计阶段调整，但必须显式定义）
