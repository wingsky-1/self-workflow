...
> **自举约束**：Self-Workflow 自身是 V1 的第一个实战项目。V1 完成后，必须能用 feat 工作流来自举开发 V2 功能。这意味着 feat 工作流的质量和 Phase Gate 的可靠性必须达到"生产可用"标准。
...
| **项目文件零侵入** | 不修改项目源代码。元数据（`.self-workflow/`）和工具配置（`.opencode/`）与源文件物理隔离 |
| **轻量 Gate** | V1 通过 Markdown Check-list 和 Review Agent 实现对抗性审查雏形 |
| **错误即时记录** | 遇到错误立刻写日志，不等工作流完成 |
| **自举优先** | Self-Workflow 自身是第一个 dogfooding 项目。所有 V1 功能必须先能在 self-workflow 仓库跑通，才能算验收通过 |
...
- [ ] **Review Agent 定义**
  ...
  - **自举价值**：基础设施代码必须经过对抗性审查才能合入，防止架构腐化
...
- [ ] **自举验证**：在 self-workflow 项目本身上用 `/feat` 完成一个真实功能开发，全流程跑通
...
- [ ] **自举场景**：当前平台是否支持项目自我引用——即用自己生成的工作流开发自己？
- [ ] 输出：审计报告，明确适配器的能力边界
